...
def test_cron_daily_at_8am_pacific():
    ...
    now_ms = int(datetime(2026, 5, 9, 14, 0, tzinfo=UTC).timestamp() * 1000)
    nxt = next_run_ms(sched, now_ms)
    assert nxt == int(datetime(2026, 5, 9, 15, 0, tzinfo=UTC).timestamp() * 1000)
...
def test_cron_weekly_monday_8am_pacific_skips_to_next_monday():
    ...
    now_ms = int(datetime(2026, 5, 12, 18, 0, tzinfo=UTC).timestamp() * 1000)
    nxt = next_run_ms(sched, now_ms)
    expected = int(datetime(2026, 5, 18, 15, 0, tzinfo=UTC).timestamp() * 1000)
    assert nxt == expected
...
def test_at_naive_iso_uses_schedule_tz():
    ...
    nxt = next_run_ms(sched)
    expected = int(datetime(2030, 1, 1, 17, 0, tzinfo=UTC).timestamp() * 1000)
    assert nxt == expected
