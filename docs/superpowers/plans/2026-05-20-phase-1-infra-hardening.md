...
> **Status (2026-08-26): partially superseded.** Rescued from the abandoned
> `feature/phase-1-infra` branch, which was never merged. The GitHub
> credential task landed separately in #42. The container/image tasks
> describe exactly the drift that `AGENTS.md` "Deploy host reality" records
> and that `scripts/reconcile-deploy-host.sh` now automates -- use those
> instead of the steps below. The Galadriel and Telegram round-trip tasks
> are still unbuilt. Do not follow the checkboxes without checking each
