...
> **Status (2026-08-26): design intent, stale on models.** Rescued from the
> abandoned `feature/phase-1-infra` branch. The product goals still hold and
> nothing here has been superseded by a later spec, but the Gemini
> references predate ADR 0006, which made Anthropic the sole provider.
> For current architecture see `README.md`, `CLAUDE.md`, and
> `docs/decisions/`.
